...
def add_flag_to_tag(link: str) -> str:
    ...
    flag = country_flag(get_country_by_ip(ip))
    
    if proto == "vmess":
        try:
            raw = link.split("://", 1)[1]
            cfg = json.loads(b64_decode(raw))
            existing_tag = cfg.get("ps", "")
            if not existing_tag:
                existing_tag = "ShatakVPN"
            if flag not in existing_tag:
                cfg["ps"] = f"{flag} {existing_tag}"
                new_b64 = b64_encode(json.dumps(cfg, ensure_ascii=False))
                return f"vmess://{new_b64}#{quote(cfg['ps'])}"
            return link
        except Exception:
            return link
    ...
    if "#" in link:
        ...
        return f"{link}#{quote(f'{flag} ShatakVPN')}"
...
def load_configs_from_file(filepath: str) -> list[str]:
    ...
    logging.info(f"Loaded {len(configs)} configs from {filepath}")
    return configs
...
async def main_async():
    ...
    logging.info(f"[{now}] Starting processing from local file…")

    config_file = "configs.txt/combined/ALL/ALL.txt"
    all_configs = load_configs_from_file(config_file)
    
    if not all_configs:
        logging.error("No configs loaded. Aborting.")
        return
    ...
    async with httpx.AsyncClient() as client:
        ...
        if not all_pairs:
            logging.warning("No valid links parsed; abort.")
            return
        ...
        flagged_global = [add_flag_to_tag(l) for l in renamed_global]

        grouped_global = group_by_protocol(sorted_global_links)

        save_to_file(os.path.join(OUTPUT_DIR, "all.txt"), flagged_global)
        save_to_file(os.path.join(OUTPUT_DIR, "light.txt"), flagged_global[:30])

        for proto, proto_links in grouped_global.items():
            renamed_proto = [rename_line(l) for l in proto_links]
            flagged_proto = [add_flag_to_tag(l) for l in renamed_proto]
            save_to_file(
        ...
        for country, _nodes in country_nodes.items():
            ...
            flagged_all_country = [add_flag_to_tag(l) for l in renamed_all_country]
            save_to_file(os.path.join(dest_dir, "all.txt"), flagged_all_country)
            save_to_file(os.path.join(dest_dir, "light.txt"), flagged_all_country[:30])
